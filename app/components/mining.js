...
class Mining {
    ...
    async startmining(port, url, nonce, blockDataHash, difficulty) {
        ...
        const {
            ...
            proofHex,
            dontMine
        } = await generateProof(nonce, blockDataHash, difficulty, zeros);
        if (dontMine === 'true') {
    ...
    addCoinBaseTransactions() {

        if (this.pendingtransactions != undefined || this.pendingtransactions.length != 0) {
            const currentTransactions = this.pendingtransactions;
            var length = currentTransactions.length;
            for (var i = 0; i < length; i++) {
                let transaction = currentTransactions[i];
                let totalReward = parseFloat(transaction.txreward) + (parseFloat(transaction.amount) * 5 / 100);
                if (totalReward > 0) {
                    let newTransaction = new TransactionClass(
                        this.generateRandomTransactionID(),
                        transaction.sender,
                        this.address,
                        totalReward,
                        "0",
                        "Coin Base Transaction for ID: " + transaction.id,
                        "No-Sign"
                    );
                ...
                // Block Reward
                let blockRewardTransaction = new TransactionClass(
                    this.generateRandomTransactionID(),
                    initialHash,
                    this.address,
                    socketActions.MINING_REWARD,
                    "0",
                    "Coin Base Transaction Block Reward",
                    "No-Sign"
                );
                this.pendingtransactions.push(blockRewardTransaction);
        }
    }
